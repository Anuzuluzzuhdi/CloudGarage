namespace CloudGarage.Meeting {
    ...
    @Serenity.Decorators.registerEditor('Meeting.MeetingAttendeeEditor')
    export class MeetingAttendeeEditor extends Serenity.Extensions.GridEditorBase<MeetingattendeeRow> {
        ...
        protected createToolbarExtensions() {
            ...
            Serenity.Widget.create({
                ...
                options: {
                    lookupKey: HumanResource.EmployeesRow.lookupKey,
                    inplaceAdd: true,
                    inplaceAddPermission: "Organization:Contacts:Management"
                },
                element: e => e.attr('placeholder', '--select contact to add--')
                    .css('float', 'left')
                ...
                init: w => w.change(x => {
                    ...
                    var employe = HumanResource.EmployeesRow.getLookup().itemById[Q.toId(w.value)];
                    w.value = null;
                    if (!employe)
                        return;

                    if (Q.any(this.getItems(), i => i.ContactId == employe.ContactId)) {
                        Q.notifyWarning("Contact is already in attendee list!");
                        return;
                    }

                    var item: MeetingattendeeRow = {
                        ContactId: employe.ContactId,
                        ContactFirstName: employe.ContactFirstName,
                    ...
                    items.push(item);
                    items.sort((a, b) => Q.turkishLocaleCompare(a.ContactFirstName, b.ContactFirstName));
                    this.setItems(items);
                })
            });
        }
