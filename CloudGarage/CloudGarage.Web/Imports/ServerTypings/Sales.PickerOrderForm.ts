namespace CloudGarage.Sales {
    export interface PickerOrderForm {
    ...
    export class PickerOrderForm extends Serenity.PrefixedContext {
        static formKey = 'Sales.PickerOrder';
        private static init: boolean;

        constructor(prefix: string) {
            super(prefix);

            if (!PickerOrderForm.init)  {
                PickerOrderForm.init = true;
                ...
                Q.initFormType(PickerOrderForm, [
                    'CustomerId', w0,
                    'Date', w1,
                    'PaymentAccount', w0,
                    'GrandTotal', w2,
